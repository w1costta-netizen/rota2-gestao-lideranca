...
function CampanhaDetalhe({ campanha: campanhaInicial, userId, profile, onBack }) {
  ...
  const saveXlsxItems = async () => {
    ...
    try {
      ...
      toast(`✅ ${toSave.length} itens importados!`);
      setXlsxModal(false);
      setXlsxItems([]);
      load();
    ...
    <div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {/* Ver flyer PDF */}
          {campanha.flyer_pdf_url && (
            <a href={campanha.flyer_pdf_url} target="_blank" rel="noreferrer"
              className="btn btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Download size={14} /> Ver Flyer
            </a>
          )}
          {isAdmin && (
            <>
              {/* Upload do flyer PDF */}
              <input ref={flyerRef} type="file" accept="application/pdf"
                style={{ display: 'none' }} onChange={handleFlyerPdf} />
              <button className="btn btn-ghost" onClick={() => flyerRef.current?.click()} disabled={flyerUploading}
                title="Enviar PDF do flyer para os colaboradores baixarem">
                {flyerUploading
              ...
              <button className="btn btn-primary" style={{ background: '#10b981' }}
                ...
                {xlsxLoading
                  ? <><Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> Lendo...</>
      ...
      <Modal open={xlsxModal} onClose={() => setXlsxModal(false)}
        title={`📊 Itens da planilha (${xlsxItems.filter(i => i.selected).length}/${xlsxItems.length} selecionados)`}>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>
          Revise os itens extraídos da planilha. Desmarque os que não devem ser importados.
        </div>
        <div style={{ maxHeight: 360, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 14 }}>
          {xlsxItems.map((it, idx) => (
            <div key={idx} style={{
              ...
              cursor: 'pointer',
            }} onClick={() => setXlsxItems(l => l.map((x, i) => i === idx ? { ...x, selected: !x.selected } : x))}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <input className="input" style={{ padding: '4px 8px', fontSize: 13, marginBottom: 4 }}
                  ...
                  onChange={e => setXlsxItems(l => l.map((x, i) => i === idx ? { ...x, descricao: e.target.value } : x))} />
                {it.categoria && <div style={{ fontSize: 11, color: 'var(--text-muted)', paddingLeft: 4 }}>{it.categoria}</div>}
              </div>
              <input className="input" style={{ width: 110, padding: '4px 8px', fontSize: 13, flexShrink: 0 }}
                value={it.preco}
                placeholder="Preço"
                onClick={e => e.stopPropagation()}
                onChange={e => setXlsxItems(l => l.map((x, i) => i === idx ? { ...x, preco: e.target.value } : x))} />
              <button className="btn-icon" style={{ color: '#ef4444', flexShrink: 0 }}
                onClick={e => { e.stopPropagation(); setXlsxItems(l => l.filter((_, i) => i !== idx)); }}>
                <X size={14} />
              </button>
            </div>
          ))}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <button style={{ fontSize: 12, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer' }}
            onClick={() => setXlsxItems(l => l.map(x => ({ ...x, selected: true })))}>Selecionar todos</button>
          <button style={{ fontSize: 12, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer' }}
            onClick={() => setXlsxItems(l => l.map(x => ({ ...x, selected: false })))}>Desmarcar todos</button>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-ghost" style={{ flex: 1, justifyContent: 'center' }} onClick={() => setXlsxModal(false)}>Cancelar</button>
          <button className="btn btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveXlsxItems} disabled={xlsxSaving}>
            {xlsxSaving ? 'Importando...' : `Importar ${xlsxItems.filter(i => i.selected).length} itens`}
          </button>
        </div>
      </Modal>
      ...
      <Modal open={itemModal} onClose={() => setItemModal(false)} title={editItem ? 'Editar item' : 'Novo item'}>
        <div className="form-group">
          ...
          <input className="input" value={itemForm.descricao}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <input className="input" value={itemForm.preco}
              onChange={e => setItemForm(f => ({ ...f, preco: e.target.value }))}
              placeholder="R$ 0,00" />
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Categoria</label>
            <input className="input" value={itemForm.categoria}
      ...
      <Modal open={bulkModal} onClose={() => setBulkModal(false)} title="Adicionar itens em lote">
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
          Cole os itens, um por linha. O preço é extraído automaticamente se estiver na linha.
        </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                    <span style={{
                      fontSize: 11, fontWeight: 700, color: TIPO_COLOR[c.tipo],
                      background: TIPO_COLOR[c.tipo] + '20', padding: '2px 8px', borderRadius: 6,
                    }}>
                      {TIPO_LABEL[c.tipo]}
                    </span>
                    {ok && <span style={{ fontSize: 11, fontWeight: 700, color: '#10b981', background: '#10b98120', padding: '2px 8px', borderRadius: 6 }}>✅ Concluído</span>}
                    {c.flyer_pdf_url && (
                      <a href={c.flyer_pdf_url} target="_blank" rel="noreferrer"
                        onClick={e => e.stopPropagation()}
                        style={{ fontSize: 11, fontWeight: 700, color: '#6366f1', background: '#6366f120',
                          padding: '2px 8px', borderRadius: 6, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Download size={10} /> Flyer PDF
                      </a>
                    )}
                  </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Nova campanha">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="form-group" style={{ margin: 0 }}>
            ...
            <input className="input" type="date" value={form.validade_ini}
              onChange={e => setForm(f => ({ ...f, validade_ini: e.target.value }))} />
          </div>
          <div className="form-group" style={{ margin: 0 }}>
            <label className="form-label">Fim da validade *</label>
            <input className="input" type="date" value={form.validade_fim}
